...
class MongoDBClient(metaclass=Singleton):
    def __init__(self) -> None:
        self.config = Config()
        if (
            self.config.get("MONGO_URL") is None
            or len(self.config.get("MONGO_URL")) == 0
        ):
            logWarning("MONGO_URL is none, all database features will be disbaled!")
            self.client = None
        else:
    ...
    def add_tgcalls_chats(self, chatInfo):
        try:
            exists = self.sktechhub.tgcalls_chats.find({"chat_id": chatInfo["chat_id"]})
