...
@Client.on_message(
    filters.command(["info", "info@vcplayerbot"]) & ~filters.edited & ~filters.bot
)
@database_check
@chat_allowed
@admin_check
async def info(client, message, current_client):
    try:
        ...
        if (
            ...
            and hasattr(message.from_user, "id")
            and message.from_user.id
            in [i["chat_id"] for i in Config.get("GLOBAL_ADMINS")]
        ):
            forceCreate = True
        ...
        status, resp_message = await pytgcalls_instance.getCurrentInfo(
            message.from_user.id
            if message.from_user is not None and hasattr(message.from_user, "id")
            else -1
        )
...
@Client.on_message(
    filters.command(["start", "start@vcplayerbot", "help", "help@vcplayerbot"])
    & ~filters.edited
)
